    ...
    QWidget(parent), scrollPos(0)
{
    ...
    setFixedHeight(fontMetrics().height()+12);
    leftMargin = height() / 3;
...
void ScrollText::doDownload()
{
    ...
    manager->get(QNetworkRequest(QUrl("http://www.amiga-news.de/de/backends/news/index.rss")));
}

void ScrollText::replyFinished (QNetworkReply *reply)
{
    QString lastTitle;
    ...
    if ( reply->error() ) {
        ...
        while (!xmlReader.atEnd()) {
            ...
            if (xmlReader.isStartElement()) {
                ...
                if ( tag == "title" ) {
                    lastTitle = xmlReader.readElementText();
                    newsList.append( lastTitle );
                }
                if ( tag == "link" ) {
                    linkList.append( QString( "%2|%1" ).arg(xmlReader.readElementText()).arg(lastTitle) );
                }
    ...
    if ( ! newsList.empty() ) {
        this->setText( newsList.join(" - ") );                      
    }
}

QStringList ScrollText::getLinkList() {
